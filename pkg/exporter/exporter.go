...
func SendStatsJSON(ctx context.Context, serverURL string, data interface{}) error {
	...
	log.Printf("Sending data (size %d bytes) to %s", len(jsonData), serverURL)
	...
	if err != nil {
		...
		if reqCtx.Err() == context.DeadlineExceeded {
			log.Printf("HTTP request to %s timed out.", serverURL)
			...
			log.Printf("HTTP request to %s cancelled by parent context: %v", serverURL, ctx.Err())
			return fmt.Errorf("http request to %s cancelled by parent context: %w", serverURL, ctx.Err())
		}
		log.Printf("Error sending stats to server %s: %v", serverURL, err)
	...
	if resp.StatusCode >= 200 && resp.StatusCode < 300 {
		log.Printf("Stats sent successfully to %s. Server responded with %s", serverURL, resp.Status)
	} else {
		log.Printf("Server at %s responded with non-OK status: %s", serverURL, resp.Status)
		responseBody, readErr := io.ReadAll(resp.Body)
		if readErr != nil {
			log.Printf("Error reading error response body from %s: %v", serverURL, readErr)
			return fmt.Errorf("server at %s responded with %s (and error reading response body: %v)", serverURL, resp.Status, readErr)
		}
		log.Printf("Server error response from %s: %s", serverURL, string(responseBody))
